...
netlify/functions/gemini.mts  Seul fichier qui parle à Google
tests/                        Tests des prompts
```
...
npm test             # vérifie que les prompts restent corrects
```
...
Lancez `npm test` après toute modification de `netlify/functions/gemini.mts`.

## Quand Google retire un modèle

C'est déjà arrivé, et cela avait mis toute l'application à l'arrêt. Les noms de
modèles sont maintenant regroupés en haut de `netlify/functions/gemini.mts`,
